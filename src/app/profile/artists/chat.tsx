...
export default function Chat({ user }: ChatProps) {
	...
	const BACKEND_URI = process.env.NEXT_PUBLIC_BACKEND_URI as string;
		...
		<Layout title={`Sillevon | Chat`}>
			<div className={styles.ChatArtist}>
				...
				<div className={styles.chatContainer}>
					<div>
						{isSelected ? (
							<ChatView currentChat={currentChat} socket={socket} userId={user._id} />
						) : (
							<WelcomeChat />
						)}
					</div>
						...
						<ul>
							{contacts.length > 0 ? (
								contacts.map((connection, i) => {
									const user = mode === 'customer' ? connection.userB : connection.userA;
									return (
