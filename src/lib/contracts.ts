import { AllSongs, RehearsalDate } from '../app/stepper-contract/page';
import { api } from './axiosCreate';

type Artists = { label: string; value: string }[];

export async function makeContracts(artists: Artists, name: string, options = {}) {
...
export async function acceptContract(id: string, options = {}) {
	const res = await api.put(`/api/contracts/accept/${id}`, { isAccepted: true }, options);
	return res.data;
}
